...
// "heightmapLowRes" is a reference to the heightmapLowRes pixels that define the shape of the mesh.
// "xStart", "yStart", "xEnd", and "yEnd" define the rectangle within the heightmapLowRes that will be included in the mesh
// "scale" is a scaling factor by which the vertex positions should be multiplied after extracting them from the heightmapLowRes.
// If the indices of a pixel are (x, y) -- integers ranging from (xStart, yStart) to (xEnd, yEnd) --
// and the value stored in the heightmapLowRes at that location is h -- an integer ranging from 0 to SHRT_MAX (2^16 - 1)
